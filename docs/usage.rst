...
The example script `run_example_4casts.py` (copied below, also on `github <https://github.com/geoffstanley/neutralocean/blob/main/neutralocean/examples/run_example_4casts.py>`_) creates synthetic salt, temperature, and pressure data for 4 vertical casts, specifies which pairs of these 4 casts are adjacent, and then calculates a potential density surface, specific volume anomaly surface, and omega surface.  Epsilon neutrality errors are calculated for each link on the final omega surface.
...
.. literalinclude:: ../neutralocean/examples/run_example_4casts.py
